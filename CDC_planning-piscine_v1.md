# Cahier des charges — Application de gestion des plannings d'intervenants (piscine)

**Version du document :** 1.0
**Destinataire :** Claude Code
**Nom de travail du projet :** `planning-piscine` (renommable)
...
### 6.2 Espace ADMIN
1. **Dashboard**
   - Prochains besoins et leur taux de remplissage (postes pourvus / total).
   - Postes **non pourvus** à venir (mise en avant).
   - **Alertes diplômes** (expirés / < 30 j).
2. **Créer un besoin**
   - Date, heure début, heure fin, commentaire.
   - Nombre de postes MNS + nombre de postes BNSSA.
3. **Liste des besoins** (onglets À venir / Passés)
   - Statut, remplissage. Clic → détail.
4. **Détail besoin**
   - Liste des postes avec niveau + intervenant assigné (ou « libre »).
   - Bouton **Libérer** par poste réservé (avec confirmation) → remet le poste à disposition.
   - Éditer / supprimer le besoin.
5. **Gestion des intervenants**
   - Liste (nom, niveau, validités, actif/inactif).
   - Créer / éditer / désactiver.
   - **Réinitialiser le mot de passe** (ressaisit un mot de passe initial + force le changement).
6. **Récap d'heures**
   - Sélection d'une période (dates).
   - Total d'heures par intervenant.
   - **Export** (CSV a minima ; PDF optionnel).
7. **Planning global** (vue liste ou calendrier simple) — lecture seule, tous les besoins.

### 6.3 Espace INTERVENANT (mobile-first)
1. **Créneaux disponibles**
   - Liste des **postes libres futurs** éligibles à son niveau (date, heures, niveau du poste, commentaire).
   - Bouton **Réserver** par poste.
   - Message clair si le poste vient d'être pris entre-temps (« déjà réservé »).
2. **Mes réservations** (À venir / Passées)
   - Rappel : pour annuler, **contacter le directeur par téléphone** (message affiché, numéro visible).
...
- **UI** : **Tailwind CSS**, **mobile-first**, responsive PC. Composants sobres
  (shadcn-svelte ou bits-ui optionnel). Pas de design lourd.
